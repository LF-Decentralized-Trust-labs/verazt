...
impl BasicBlock {
    ...
    pub fn compute_def_use(&mut self) {
        ...
        if let Terminator::Branch { condition, .. } = &self.terminator {
            let uses = crate::frameworks::dfa::utils::get_vars_in_expr(condition);
            for var in uses {
                if !local_defs.contains(&var) {
                    self.upward_exposed.insert(var.clone());
                }
...
pub struct ControlFlowGraph {
    ...
    domtree: Option<crate::frameworks::cfa::domtree::DomTree>,
}
...
impl ControlFlowGraph {
    ...
    pub fn domtree(&self) -> Option<&crate::frameworks::cfa::domtree::DomTree> {
        self.domtree.as_ref()
    }
...
fn collect_used_vars(stmt: &Stmt) -> Vec<VarId> {
    crate::frameworks::dfa::utils::collect_used_vars(stmt)
}
