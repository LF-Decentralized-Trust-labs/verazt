//! Enhanced CFG with def/use annotations for dataflow analysis.
//!
...
impl BasicBlock {
    ...
    pub fn compute_def_use(&mut self) {
        ...
        if let Terminator::Branch { condition, .. } = &self.terminator {
            let uses = crate::analysis::frameworks::dfa::utils::get_vars_in_expr(condition);
            for var in uses {
                if !local_defs.contains(&var) {
                    self.upward_exposed.insert(var.clone());
                }
...
pub struct ControlFlowGraph {
    ...
    domtree: Option<crate::analysis::frameworks::cfa::domtree::DomTree>,
}
...
impl ControlFlowGraph {
    ...
    pub fn set_domtree(&mut self, dom: crate::analysis::frameworks::cfa::domtree::DomTree) {
        self.domtree = Some(dom);
    }

    /// Get the attached dominator tree, if any.
    pub fn domtree(&self) -> Option<&crate::analysis::frameworks::cfa::domtree::DomTree> {
        self.domtree.as_ref()
    }
...
fn collect_used_vars(stmt: &Stmt) -> Vec<VarId> {
    crate::analysis::frameworks::dfa::utils::collect_used_vars(stmt)
}

/// Collect variables defined in a statement
fn collect_defined_vars(stmt: &Stmt) -> Vec<VarId> {
    crate::analysis::frameworks::dfa::utils::collect_defined_vars(stmt)
}
