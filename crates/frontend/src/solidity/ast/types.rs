...
use crate::solidity::ast::{DataLoc, Name};
use num_bigint::BigInt;
use std::fmt::{self, Display};

/// Variable control printing data location for the [`Display`] trait.
pub static mut PRINT_DATA_LOC: bool = true;

//-------------------------------------------------------------------------
// Data structures representing all types
//-------------------------------------------------------------------------
...
pub struct ContractType {
    pub name: Name,
    pub is_lib: bool,
    pub scope: Option<Name>, // Where the contract type is defined.
}

#[derive(PartialEq, Eq, Debug, Clone, Hash)]
pub struct EnumType {
    pub name: Name,
    pub scope: Option<Name>, // Where the enum type is defined.
}
...
pub struct StructType {
    pub name: Name,
    pub scope: Option<Name>, // Where the struct type is defined
    pub data_loc: DataLoc,
    pub is_ptr: bool, // Whether it is a pointer type.
}
...
impl Type {
    ...
    pub fn name(&self) -> Option<Name> {
        match self {
            ...
            Type::UserDefined(t) => Some(t.name.clone()),
    ...
    pub fn update_name(&mut self, name: Name) {
        match self {
            ...
            Type::UserDefined(t) => t.name = name,
    ...
    pub fn update_scope(&mut self, scope: Option<Name>) {
        match self {
            Type::Struct(t) => t.scope = scope,
            Type::Enum(t) => t.scope = scope,
            Type::UserDefined(t) => t.scope = scope,
            Type::Contract(t) => t.scope = scope,
            _ => {}
        }
    }
    ...
    pub fn is_sub_type(&self, other: &Type) -> bool {
        match (self, other) {
            ...
            (Type::Fixed(_), Type::Fixed(_)) => true,
            (Type::String(_), Type::String(_)) => true,
            (Type::Address(_), Type::Address(_)) => true,
            (Type::Bytes(_), Type::Bytes(_)) => true,
            (Type::Array(t1), Type::Array(t2)) => t1.is_sub_type(t2),
            (Type::Slice(t1), Type::Slice(t2)) => t1.is_sub_type(t2),
            (Type::Struct(_), Type::Struct(_)) => true,
            (Type::Enum(_), Type::Enum(_)) => true,
            (Type::Module(_), Type::Module(_)) => true,
            (Type::Tuple(t1), Type::Tuple(t2)) => t1.is_sub_type(t2),
            (Type::Func(t1), Type::Func(t2)) => t1.is_sub_type(t2),
            (Type::Mapping(t1), Type::Mapping(typ2)) => t1.is_sub_type(typ2),
            (Type::UserDefined(_), Type::UserDefined(_)) => true,
            (Type::Contract(_), Type::Contract(_)) => true,
            (Type::Magic(t1), Type::Magic(typ2)) => t1.is_sub_type(typ2),
            _ => false,
        }
    }
    ...
    pub fn set_data_loc(&mut self, dloc: DataLoc) {
        match self {
            Type::Array(t) => t.data_loc = dloc,
            Type::String(t) => t.data_loc = dloc,
            Type::Bytes(t) => t.data_loc = dloc,
            Type::Struct(t) => t.data_loc = dloc,
            Type::Mapping(t) => t.data_loc = dloc,
            _ => {}
        }
    }
}
...
impl ArrayType {
    pub fn new(base: Type, length: Option<BigInt>, dloc: DataLoc, is_ptr: bool) -> Self {
        ArrayType { base: Box::new(base), length, data_loc: dloc, is_ptr }
    }

    pub fn is_sub_type(&self, other: &ArrayType) -> bool {
        self.base.is_sub_type(&other.base)
    }
}
...
impl Display for ArrayType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        unsafe {
            ...
            let saved_display_data_loc = PRINT_DATA_LOC;

            // Do not print data location in base type
            PRINT_DATA_LOC = false;
            match &self.length {
                None => write!(f, "{}[]", self.base).ok(),
                Some(len) => write!(f, "{}[{}]", self.base, len).ok(),
            };

            // Restore `PRINT_DATA_LOC` flag
            PRINT_DATA_LOC = saved_display_data_loc;
            if PRINT_DATA_LOC && self.data_loc != DataLoc::None {
...
impl BytesType {
    pub fn new(length: Option<u8>, dloc: DataLoc, is_ptr: bool) -> Self {
        BytesType { length, data_loc: dloc, is_ptr }
    }
...
impl Display for BytesType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        unsafe {
            if PRINT_DATA_LOC && self.length.is_none() && self.data_loc != DataLoc::None {
                write!(f, " {}", self.data_loc).ok();
            }
        }
...
impl ContractType {
    pub fn new(name: Name, is_lib: bool, scope: Option<Name>) -> Self {
        ContractType { name, is_lib, scope }
    }
...
impl FuncType {
    ...
    pub fn is_sub_type(&self, other: &FuncType) -> bool {
        ...
        match self.returns.len() == other.returns.len() {
            true => {
                for (param1, param2) in self.returns.iter().zip(other.returns.iter()) {
                    let param1 = (*param1).clone();
                    let param2 = (*param2).clone();
                    if !param1.is_sub_type(&param2) {
                        return false;
...
impl Display for FuncType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        unsafe {
            ...
            // Print data location for function type
            PRINT_DATA_LOC = true;
...
//-------------------------------------------------------------------------
// Implementations for Magic type
//-------------------------------------------------------------------------
...
impl MagicType {
    ...
    pub fn new_meta_type(sub_type: Type) -> Self {
        MagicType::MetaType(Box::new(sub_type))
    }

    pub fn is_sub_type(&self, other: &MagicType) -> bool {
        match (self, other) {
            (MagicType::BlockType, MagicType::BlockType) => true,
            (MagicType::MessageType, MagicType::MessageType) => true,
            (MagicType::TxnType, MagicType::TxnType) => true,
            (MagicType::ABIType, MagicType::ABIType) => true,
            (MagicType::MetaType(typ1), MagicType::MetaType(typ2)) => typ1.is_sub_type(typ2),
            _ => false,
        }
    }
}
...
impl MappingType {
    pub fn new(key: Type, value: Type, dloc: DataLoc) -> Self {
        MappingType { key: Box::new(key), value: Box::new(value), data_loc: dloc }
    }

    pub fn is_sub_type(&self, super_type: &MappingType) -> bool {
        self.key.is_sub_type(&super_type.key) && self.value.is_sub_type(&super_type.value)
    }
}
...
//-------------------------------------------------------------------------
// Implementations for Slice type
//-------------------------------------------------------------------------
...
impl SliceType {
    ...
    pub fn is_sub_type(&self, super_type: &SliceType) -> bool {
        self.base.is_sub_type(&super_type.base)
    }
...
impl StringType {
    pub fn new(dloc: DataLoc, is_ptr: bool) -> Self {
        StringType { data_loc: dloc, is_ptr }
    }
}
...
impl StructType {
    pub fn new(name: Name, scope: Option<Name>, dloc: DataLoc, is_ptr: bool) -> Self {
        StructType { name, scope, data_loc: dloc, is_ptr }
    }
}
...
impl TupleType {
    ...
    pub fn is_sub_type(&self, super_type: &TupleType) -> bool {
        if self.elems.len() != super_type.elems.len() {
            return false;
        }

        for (typ1, typ2) in self.elems.iter().zip(super_type.elems.iter()) {
            match (typ1, typ2) {
                (None, None) => {}
                (None, Some(_)) => return false,
                (Some(_), None) => return false,
                (Some(typ1), Some(typ2)) => {
                    if !typ1.is_sub_type(typ2) {
                        return false;
                    }
                }
            }
