...
use crate::solidity::analysis::pass::{AnalysisPass, Pass, PassResult};
...
pub struct ModifierInfo {
    ...
    /// Location in source.
    pub loc: Option<crate::solidity::ast::Loc>,
}
        ...
        let modifiers: Vec<Name> = func
            ...
            .filter_map(|m| {
                ...
                match m.callee.as_ref() {
                    crate::solidity::ast::Expr::Ident(ident) => Some(ident.name.clone()),
                    _ => None,
                }
            })
            .collect();
