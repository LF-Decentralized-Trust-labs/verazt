...
impl AnalysisContext {
    ...
    pub fn contracts(&self) -> Vec<&crate::solidity::ast::ContractDef> {
        self.source_units
            .iter()
            .flat_map(|su| su.elems.iter())
            .filter_map(|elem| {
                if let crate::solidity::ast::SourceUnitElem::Contract(c) = elem {
                    Some(c)
                } else {
                    None
                }
    ...
    pub fn functions(&self) -> Vec<&crate::solidity::ast::FuncDef> {
        let mut funcs = Vec::new();

        for su in &self.source_units {
            for elem in &su.elems {
                match elem {
                    crate::solidity::ast::SourceUnitElem::Func(f) => funcs.push(f),
                    crate::solidity::ast::SourceUnitElem::Contract(c) => {
                        for body_elem in &c.body {
                            if let crate::solidity::ast::ContractElem::Func(f) = body_elem {
                                funcs.push(f);
                            }
                        }
                    }
