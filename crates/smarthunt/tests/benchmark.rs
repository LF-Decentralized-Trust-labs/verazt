//! Integration tests: benchmark smarthunt against the SmartBugs-curated
//! dataset.
//!
//! These tests use `smartbench` as a dev-dependency to run smarthunt
//! programmatically against the dataset and verify detection accuracy.

use bugs::bug::BugCategory;
use smartbench::runner::DetectedBug;
use smartbench::{generate_report, match_file, scan_dataset};
use smarthunt::{AnalysisConfig, AnalysisContext, PipelineConfig, PipelineEngine};
use solidity::parser::parse_input_file;
use std::path::Path;

/// Run smarthunt on a single .sol file and return DetectedBugs.
fn run_smarthunt_on_file(file_path: &Path) -> Vec<DetectedBug> {
...
fn test_reentrancy_detection_accuracy() {
    ...
    for (file_path, file_annotations) in &annotations_by_file {
        let detections = run_smarthunt_on_file(file_path);
