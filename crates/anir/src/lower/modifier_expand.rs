...
//! positions. Only runs when `#scir.loaded_dialects` contains `"evm"`.

use scir::{ContractDecl, FunctionDecl, MemberDecl, Stmt};
...
pub fn expand_modifiers(func: &FunctionDecl, contract: &ContractDecl) -> Vec<Stmt> {
    ...
    let modifier_defs: Vec<_> = contract
        ...
        .iter()
        .filter_map(|m| {
            if let MemberDecl::Dialect(scir::DialectMemberDecl::Evm(
                scir::dialect::evm::EvmMemberDecl::ModifierDef { name, body, .. },
            )) = m
            {
    ...
    // Since SCIR doesn't have an explicit placeholder statement, we simply
