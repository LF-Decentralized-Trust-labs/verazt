...
//!
//! Converts structured SCIR statements into basic blocks with explicit
//! control flow edges (terminators).

use crate::cfg::{BasicBlock, BlockId, Terminator};
use crate::ops::{Op, OpId, OpKind, OpRef, SsaName};
use scir::{Expr, Param, Stmt, Type};
...
/// Build a CFG from a list of SCIR statements (function body).
    ...
    match stmt {
        ...
        Stmt::AugAssign(aug) => {
            ...
            let kind = OpKind::BinOp {
                ...
                lhs: lhs_ref,
                rhs: rhs_ref,
                overflow: scir::OverflowSemantics::Checked,
            };
    ...
    match expr {
        ...
        Expr::Var(_) => {
            ...
            OpKind::PseudoValue {
                ...
                taint: crate::interfaces::TaintLabel::Clean,
            }
        }
        Expr::BinOp(binop) => {
