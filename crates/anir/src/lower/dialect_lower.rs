...
//!
//! Each dialect construct in SCIR is lowered to ANIR ops that implement
...
fn detect_dialect(attrs: &[Attr]) -> String {
    for attr in attrs {
        if attr.namespace == "scir" && attr.key == "loaded_dialects" {
            if let scir::AttrValue::String(s) = &attr.value {
                return s.clone();
            }
        }
    }
